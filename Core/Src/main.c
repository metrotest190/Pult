...
/* Порог «испытание идёт». Раньше был 0.5 — при разрыве тонких материалов
   (пик F = 0.2) срыв детектировался верно, но фиксация блокировалась этим
   порогом, и экран продолжал показывать текущие L/D при F = 0. Порог снижен
   до уровня, заведомо превышающего шум Тензо датчика у нуля. */
#define F_PEAK_MIN_FORCE   0.02f    /* ниже — испытание не началось, пик не считаем */
#define F_PEAK_DROP_FRAC   0.02f    /* спад на 2% от максимума = срыв */
/* Нижняя граница допуска срыва. Раньше была 0.05 — при малой силе она
   раздувала «плато» у максимума: при Fmax = 0.2 допуск составлял 25% пика,
   и L/D продолжали обновляться ещё ~20% пути после разрыва. */
#define F_PEAK_DROP_ABS    0.005f   /* ...но не менее этой абсолютной величины */
...
void ProcessButtons(void) {
    ...
    /* Сброс зафиксированного результата испытания — по нажатию ЛЮБОЙ кнопки
       пульта (раньше сбрасывали только «запись нуля» PA12 и «пуск» PB9, и
       оператору было неудобно искать нужную). Поворот энкодера сброса НЕ
       даёт: его сигналы A/B исключены из PORT_B_BUTTON_MASK. */
    if ((pressed_A | pressed_B) != 0U) {
        ResetForcePeak();
    }
